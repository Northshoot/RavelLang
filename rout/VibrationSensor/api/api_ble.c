...
void vibration_model__sys_write_to_BLE(ble_os_t *p_ravel_service, VibrationModel *model) {

	if (p_ravel_service->conn_handle != BLE_CONN_HANDLE_INVALID) {
		uint16_t len = sizeof(VibrationModel);
		ble_gatts_hvx_params_t hvx_params;
		memset(&hvx_params, 0, sizeof(hvx_params));
		hvx_params.handle = p_ravel_service->vibration_model_handles.value_handle;
...
inline void vibration_model__data_flow_event(ble_os_t * p_ravel_service, ble_evt_t * p_ble_evt){
    ble_gatts_evt_write_t * p_evt_write = &p_ble_evt->evt.gatts_evt.params.write;
    NRF_LOG_INFO("vibration_model__data_flow_event \r\n");
...
static void on_ble_write(ble_os_t * p_ravel_service, ble_evt_t * p_ble_evt) {
	...
	if (p_evt_write->handle == p_ravel_service->vibration_model_handles.cccd_handle) {
         vibration_model__data_flow_event(p_ravel_service, p_ble_evt);
	}else if (p_evt_write->handle == p_ravel_service->p_phase_model_handles.value_handle){
            p_phase_model__data_flow_event(p_ravel_service, p_ble_evt);else if (p_evt_write->handle == p_ravel_service->compressed_sample_model_handles.value_handle){
...
static uint32_t vibration_model__characteristic_add(ble_os_t * p_ravel_service) {
	uint32_t err_code = 0; // Variable to hold return codes from library and softdevice functions

	// OUR_JOB: Step 2.A, Add a custom characteristic UUID
	ble_uuid_t char_uuid;
	ble_uuid128_t base_uuid = BLE_UUID_RAVEL_BASE_UUID;
	char_uuid.uuid = BLE_UUID_VIBRATION_MODEL_CHAR_UUID;
	...
	attr_char_value.max_len = sizeof(VibrationModel);
	attr_char_value.init_len = 0;
	attr_char_value.p_value = NULL;

	// OUR_JOB: Step 2.E, Add our new characteristic to the service
	err_code = sd_ble_gatts_characteristic_add(p_ravel_service->service_handle,
			&char_md, &attr_char_value, &p_ravel_service->vibration_model_handles);
	APP_ERROR_CHECK(err_code);
...
void ravel_service_init(ble_os_t * p_ravel_service) {
	...
	// OUR_JOB: Call the function our_char_add() to add our new characteristic to the service.
	 vibration_model__characteristic_add(p_ravel_service);
