 ...
 */
static uint32_t rx_char_add(ble_rad_t * p_rad,  ble_rad_init_t * p_rad_init)
{
 ...
 * @return NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t tx_char_add(ble_rad_t * p_rad,  ble_rad_init_t * p_rad_init)
{
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_t    attr_char_value;
    ble_uuid_t          ble_uuid;
...
static
void rad_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{

}

uint32_t
ble_rad_string_send(ble_rad_t * p_rad, uint8_t * p_string, uint16_t length)
{
...
uint32_t ble_rad_init()
{
    ...
    nus_init.data_handler = rad_data_handler;
...
//    VERIFY_PARAM_NOT_NULL(m_rad_init);
    ...
    err_code = rx_char_add(&m_rad, &m_rad_init);
    VERIFY_SUCCESS(err_code);

    // Add the TX Characteristic.
    err_code = tx_char_add((&m_rad, &m_rad_init);
    VERIFY_SUCCESS(err_code);
