...
void config_ctr__startTimerPeriodic(uint32_t period){
	uint32_t time =  APP_TIMER_TICKS(period,  APP_TIMER_PRESCALER) ; 		 //pre-scale should a parameter
	app_timer_start(config_ctr__startTimerPeriodic, time,  NULL);
}
  
 void config_ctr__stopTimerPeriodic(uint32_t period){
	app_timer_stop(config_ctr__stopTimerPeriodic); // 
}
...
void timers_init(void){
    ...
    app_timer_create(&, APP_TIMER_MODE_REPEATED, config_ctr__timer_periodic__expired);
